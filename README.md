...
| "**수강 취소**가 발생하면 이미 계산된 정산 금액에서 **환불분이 차감**" | 명세가 "결제 취소(전액 환불)"와 "수강 도중 부분 환불"을 구분하지 않음 → 두 케이스를 **환불액(`refundAmount`)이 발생하는 동일 사건**으로 통합 해석. `SaleRecord`에 별도 상태(`cancelled` 등)를 두지 않고, `CancelRecord` 누적합으로 환불 상황을 표현. 자세한 해석은 아래 부연 참조. |

명세에 명시되지 않은 지점(특히 **음수 월의 수수료**)은 임의로 채우지 않고 정책을 명시적으로 결정하고 근거를 남기는 방식으로 처리했다. 다음 절 참조.

### "수강 취소" 용어 해석 (모호 → 통합 모델)

과제 원문의 "수강 취소"는 모호한 표현이다. 두 가지 케이스가 가능:

1. **결제 취소 (전액 환불)** — 수강 시작 전 변심 등으로 결제 자체를 무효화. `refundAmount == 원결제`.
2. **수강 도중 부분 환불** — 강의를 일부 수강하다가 그만두고 일부 환불. `refundAmount < 원결제`.

명세는 두 케이스를 구분하지 않고 `CancelRecord(saleId, refundAmount, cancelledAt)` 단일 모델로 묶어두었다. 즉 **"취소 = 환불액이 발생한 사건"** 이라는 단순 모델이 명세의 의도로 보인다.

본 구현은 이 의도를 그대로 따라:
- `SaleRecord`에 별도 상태 필드(예: `SOLD / CANCELLED`)를 두지 않음.
- "취소"는 라이프사이클 상태 전이가 아니라 **환불액 누적 사건**으로 표현.
- `CancelRecord` 누적합 == 원결제면 사실상 "전액 취소", 누적합 < 원결제면 "부분 환불 진행 중", 누적합 > 원결제는 거부(422).
- 정산 계산은 이 두 케이스를 **구분 없이 동일하게** 처리한다. 그 달의 환불액 합 → 순판매에서 차감.

| 시나리오 | 데이터 입력 | 본 구현의 처리 |
|---|---|---|
| 결제 취소 (전액) | `refundAmount = 80,000` (원결제 = 80,000) | 정산에서 80,000 차감 |
| 수강 도중 부분 환불 | `refundAmount = 30,000` (원결제 = 80,000) | 정산에서 30,000 차감 |
| 분할 환불 (2회) | `refundAmount = 50,000` → `refundAmount = 30,000` (같은 saleId) | 누적 80,000 차감 |
| 누적 초과 시도 | 누적 90,000 > 원결제 80,000 | **422 `REFUND_EXCEEDS_PAYMENT`** |

**운영 확장 시**: 실서비스에서는 "취소 사유"(수강생 변심 / 강의 품질 / 환불 어뷰징 등)와 "환불 시점"(수강 시작 전 / 진도율 N% / 수강 종료 후)에 따라 정책 분기가 필요하다. 본 과제는 그 분기를 명세하지 않았으므로 **단일 모델로 통합**했고, 향후 `CancelRecord` 에 `reason` / `cancelledStage` 컬럼 추가 + 정책 분기 도입 가능하도록 열려 있다.
...
### 2. 음수 월 수수료: 클램프(비환불) 정책 — **과제 단순화 채택**

환불이 그 달 판매보다 큰 달은 순판매가 **음수**가 된다. 이때 수수료를 어떻게 산정할지가 평가 변별 지점이며, 명세가 침묵하는 underspecified 지점이다.
...
**의미: 수수료 비환불(non-refundable) 정책.** 판매 월에 부과한 20% 수수료는 환불이 발생해도 환원하지 않는다. 즉 환불액 전체가 크리에이터 정산에서 차감되고, 플랫폼은 이미 받은 수수료를 유지한다. 그래서 환불 월의 수수료는 0, payout은 음수(크리에이터에게서 차감).

> **수강생에게 가는 환불액은 항상 100%다.** 비환불 정책의 "수수료"는 *플랫폼이 이미 가져간 자기 수수료를 환원하지 않는다*는 뜻이며, 수강생 환불액과는 무관하다. 차이는 "환불 손실을 누가 부담하느냐"의 문제다.

**왜 "음수를 0으로 클램프"가 아닌가**: 환불액이 증발해서 누적 정산 검산이 깨진다. 음수 = "이번 달 차감(클로백) 금액"으로 그대로 노출하는 것이 회계적으로 정합.

#### ⚠️ 본 정책은 과제 단순화 — 실서비스와 다름을 명시

위 비환불 정책을 채택한 이유는 **과제 명세에 환불 정책의 비즈니스 분기가 없기 때문**이다. 과제는 `refundAmount` 단일 입력만 받고(과제 §2.4: 실제 결제 시스템 연동 불필요), 환불 사유·기간·어뷰징 분류 등을 다루지 않는다. 그래서 데이터 모델에 "100% 차감"이 자연스럽게 들어맞고, 정책 근거가 한 줄 규칙(`max(net,0) × 0.2`)으로 깔끔하게 정리된다.

**실제 강의 플랫폼들(Udemy/Coursera/Patreon/네이버 스마트스토어 등)은 대부분 환원(refundable) 정책 + 추가 어뷰징 방어 장치 조합으로 운영한다.** 비환불을 단독으로 쓰면:
- 크리에이터가 통제 불가능한 변수(수강생 변심, CS 사고)에 대한 손실을 일방적으로 부담
- 플랫폼이 환불을 줄일 인센티브가 없어짐 → CS 품질/환불 정책 관리 동기 저하
- 악성 환불 어뷰징의 방어 부담이 크리에이터에게 전가

**운영 확장 시 추가해야 할 분기 (본 과제 범위 밖)**:
- **환불 사유 분류**: 정상 환불은 환원 모델, 어뷰징 의심은 보류 풀로 분리
- **환불 기간/조건 제한**: Udemy 30일, 수강 진도율 50% 이상 차단 등
- **PG 결제 수수료는 환불 시에도 비환불** (Stripe 표준) — 처리 원가성 비용은 사용자 부담
- **악성 환불자 차단** (블랙리스트, ML 기반 어뷰징 탐지)
- **플랫폼 보전 풀**: 환불 손실 일부를 플랫폼이 부담

본 구현은 과제 명세에 없는 분기를 임의로 만들지 않기 위해 **단일 정책(비환불 + 클램프)으로 유지**했고, 향후 환불 사유 컬럼 추가 → 사유별 정책 분기 → 어뷰징 탐지 파이프라인 순으로 확장 가능하도록 설계가 열려 있다.
...
본 과제는 **기획 → 구현 → 검증 3단계 워크플로우**로 진행했으며, 각 단계에 활용한 AI 도구와 본인 역할을 명시한다.

### 1단계 — 기획 (Claude + GPT 교차검증)

- **Claude**(일반 대화)로 정산 도메인을 분석하고 작업지시서 초안 작성
  - 정산 비즈니스 규칙 명세 작성 (환불 귀속 / 음수 월 / 월 경계 / 누적 환불 / 타임존 등)
  - 도메인 모델 / ERD 초안
  - 엣지케이스 시나리오 도출 (특히 명세가 침묵하는 underspecified 지점 식별)
  - 구현 우선순위(과설계 방지) 및 가산점 범위 가이드
- **GPT**로 작업지시서를 **2회 교차검증** (v1 → v2 → v3)
  - v2 반영: 음수 월 수수료 정책(클램프/비환불 확정), 누적 환불 초과 방어, 금액 `long` / 시각 `Instant` + KST 분리, 정규화 유지, 범위 관리 우선순위 신설
  - v3 반영: 구현 품질 가이드(순수 계산기 분리, 테스트 네이밍, validation 계층 분리, 예외 응답 포맷·에러 코드 세트, README 한 줄들)

→ 결과물: 정책·ERD·우선순위·테스트 케이스·README 요구사항이 모두 박힌 **작업지시서 (Markdown 문서)**. 코드를 직접 짜기 전에 "무엇을 / 왜 / 어떻게" 가 글로 확정된 상태.

### 2단계 — 구현 (Claude Code에 작업지시서 전달)

작업지시서 문서를 **Claude Code**에 전달하여 다음을 구현:
- Spring Boot 프로젝트 스캐폴딩 (Gradle, application.yml)
- 도메인 엔티티 5종 + JPA 리포지토리 5종
- `SettlementCalculator`(순수 POJO) ↔ `SettlementService`(조회) 분리
- 4개 REST 컨트롤러 + 8개 DTO + 9종 에러 코드 + `@RestControllerAdvice` 통일 응답
- 시드 데이터 로더 (과제 §6.1 + 작업지시서 §6.2 역산 cancel 3종)
- 단위 테스트 7개 + 통합 테스트 21개 (= 총 **28개**)
- Swagger UI 통합 (`springdoc-openapi`)
- README 본문 (정책 결정 사항, ERD, API 예시 등)

### 3단계 — 검증·수정 (본인)

Claude Code 산출물에 대해 본인이 직접 수행:
- **모든 정책 최종 결정**: 음수 월 비환불 채택, 율 적용 정책("정산월 기준 단일 율"), 권한 분리(URL 컨벤션 수준) 등 — Claude Code가 제안한 옵션 중 본인이 선택·확정.
- **산출물 검증·수정 지시**: 코드 리뷰 후 보강 지시 (예: 율 호이스팅 + 주석, README 엣지케이스 표를 "위험 → 대처" 관점으로 재작성, "수강 취소" 용어 해석 명시, "비환불 정책 ≠ 산업 표준" 솔직 톤 정정 등).
- **추가 시나리오 / 엣지케이스 정의**: 미래 월 / 운영자 집계 `to<from` / 누적 환불 초과 / 잘못된 saleId·courseId / amount 0 등 7개 추가 케이스 정의 → Claude Code가 테스트로 구현.
- **라이브 검증**: 매 단계마다 `./gradlew test` (28개 그린 확인) + `bootRun` + curl 호출로 검증표 수치(260K / 110K / 150K / 30K / 120K / 4건·2건) 직접 확인.
- **면접 방어 한 줄 준비**: 율 고정 / N+1 / 비환불 vs 환원 / 수강 취소 용어 등 깊이 있는 질문에 대한 답변 준비.

### 정리

| 단계 | 도구 | 산출물 | 본인 역할 |
|---|---|---|---|
| 기획 | Claude + GPT 교차검증 | 작업지시서 (v3) | 정책 결정 / 우선순위 판단 |
| 구현 | Claude Code | 코드 + 테스트 + README 본문 | 산출물 검증·수정 지시 |
| 검증 | (본인) | 테스트 그린 + 라이브 호출 | 모든 정책의 최종 책임 |

**AI가 짠 코드를 그대로 가져다 쓴 게 아니라, 정책·트레이드오프 의사결정과 검증은 모두 본인의 책임**으로 두는 워크플로우다. AI는 "정책을 글로 정한 작업지시서"와 "코드로 옮기는 실행기" 두 단계로 역할을 분리해, AI 산출물이 본인의 정책 의도를 정확히 따랐는지 검증 가능하게 했다.

**검증된 결과**: 모든 정산 수치는 단위 7 + 통합 21 = **28개 테스트로 자동 검증**되며 1원이라도 어긋나면 빌드가 깨진다. 검증표 수치는 단위 테스트(`should_match_assignment_validation_table_for_creator1_march`)와 통합 테스트(`should_match_validation_table_for_creator1_march_2025`) 양쪽에서 `is(X)` tight assert.
...
- **수수료 정책 (과제 단순화)**: 수수료는 **비환불(non-refundable)** 정책으로 가정하여 `commission = max(netSales, 0) × 0.2`로 계산하고, 환불만 있는 달은 수수료 0 · 정산예정 음수(차감)로 처리했다. 실서비스 강의 플랫폼은 보통 환원 모델 + 어뷰징 방어 장치 조합을 쓰지만, 과제 명세에 환불 사유·기간 분기가 없어 단일 정책으로 유지했다(§설계 결정 2 참조).
