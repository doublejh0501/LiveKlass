...
### 통합 테스트 (`SettlementIntegrationTest` × 14)
...
작업지시서 §7.2를 따라 다음 케이스를 추가 등록·검증했다:

1. **음수 정산 월** — 환불만 있는 달이 실제로 발생할 수 있는 시나리오(샘플 데이터의 cancel-3). 정책(수수료 비환불)을 코드로 증명.
2. **월 경계 정밀도** — `23:59:59` 닫힘 비교가 아닌 반열린 구간이 정확함을 증명.
3. **타임존 함정** — UTC 입력이 KST 기준 다른 달로 넘어가는 경우.
4. **누적 환불 초과** — 1차 환불 OK → 2차 시도가 누적으로 초과 → 422. 실서비스 사고 패턴 방어.
5. **잘못된 연월 형식 (3종)** — 자릿수·구분자·월 범위 모두 strict 파싱으로 400.
...
### 인증/인가
과제 명시에 따라 간략 처리 — `creatorId`는 URL 경로 파라미터로 전달. 실서비스에서는 JWT/세션 기반으로 교체.
